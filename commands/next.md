...
3. Если `mode === "obsidian"` → используй Obsidian path resolution из SKILL.md для всех файловых операций

**Obsidian mode path mapping:**
...
- STATUS.md → `<vault>/Plans/<Plan Name>/Status.md`
- CHANGELOG.md → `<vault>/Plans/<Plan Name>/Changelog.md`
- INDEX.md → `<vault>/Plans/Plans MOC.md`

**Obsidian mode status operations:**
- Чтение статуса: парси YAML frontmatter поле `status:`
- Запись статуса: обнови `status:` в frontmatter + `updated:` на текущую дату + `tags:` (замени `part/ready` → `part/in_progress`)
...
2. Обнови MASTER.md: статус части в таблице
   > **Obsidian mode**: обнови frontmatter `status: in_progress` в Part note + `updated:` дату + тег `part/in_progress`
...
5. Обнови STATUS.md и INDEX.md
   > **Obsidian mode**: Dataview в Status.md обновляется автоматически из frontmatter. Обнови только frontmatter в Part note и Plan Overview.

...
```
Разблокированные части с агентами:
→ 05-monitoring (ready, Agent: auto, Size: M)
→ 06-deployment (ready, Agent: tdd-guide, Size: S)

Запустить агентов? (y — все / n — нет / номер — конкретную)
```

- Если пользователь подтверждает → процедура "Launch Agent" или "Parallel Agent Launch" из SKILL.md
- Если отклоняет → просто показать `Next ready:` как обычно

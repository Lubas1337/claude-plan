...
### Step 0: Detect Backend (для инъекции)

Проверь `.plan/config.json`:
- Если `backend === "obsidian"` → добавь в инъекцию (Шаг 3) доп. строки про Obsidian mode
- Если `local` или нет config → стандартная инъекция без изменений
...
1.5. Прочитай `.plan/PROJECT.md` (если есть) — проектный контекст и roadmap
